...
def main():
    ...
    parser.add_argument("--extra-val-ratio", type=float, default=0.1,
                        help="Fraction of extra data songs held out for val (ignored if split files exist)")
    parser.add_argument("--pos-weight-cap", type=float, default=10.0,
                        help="Cap pos_weight values to prevent extreme imbalance (0=no cap)")
    ...
    # Load extra data directories.
    # If split-specific .tab files exist (e.g. data_wikimeter_train.tab),
    # use those directly. Otherwise fall back to random stratified split.
    if args.extra_data:
        import csv as _csv
        import random
        import re
        ...
        def _parse_tab_entries(tab_file: Path) -> list[tuple[Path, list[int]]]:
            entries: list[tuple[Path, list[int]]] = []
        ...
        for extra_dir in args.extra_data:
            ...
            # Check for pre-split .tab files (generated by download_wikimeter.py --tab-only)
            split_files = {}
            for split_name in ("train", "val", "test"):
                candidates = sorted(extra_dir.glob(f"data_*_{split_name}.tab"))
                if candidates:
                    split_files[split_name] = candidates[0]

            if "train" in split_files:
                # Use pre-split files (hash-based deterministic split)
                for split_name, target_list in [("train", train_entries), ("val", val_entries), ("test", test_entries)]:
                    if split_name in split_files:
                        split_data = _parse_tab_entries(split_files[split_name])
                        target_list.extend(split_data)
                        print(f"  Extra {split_name}: +{len(split_data)} from {split_files[split_name].name}")
                continue

            # Fallback: single .tab file with random stratified split
            tab_files = sorted(
                p for p in extra_dir.iterdir()
                if p.suffix in (".tab", ".csv", ".tsv")
            )
            for tab_file in tab_files:
                extra_entries = _parse_tab_entries(tab_file)
                ...
                for meter, songs_list in sorted(meter_songs.items()):
                    random.shuffle(songs_list)
                    if len(songs_list) <= 1:
                        n_val = 0
                    else:
                        n_val = max(1, int(len(songs_list) * args.extra_val_ratio))
                        n_val = min(n_val, len(songs_list) - 1)
                    val_songs.update(songs_list[:n_val])
    ...
    if args.pos_weight_cap > 0:
        pos_weights = pos_weights.clamp(max=args.pos_weight_cap)
    pos_weights = pos_weights.to(device)
    print(f"Pos weights: {[f'{w:.2f}' for w in pos_weights.tolist()]}"
          f"{f' (capped at {args.pos_weight_cap})' if args.pos_weight_cap > 0 else ''}")
